...
app.controller('DeliveryController', ['$scope', '$http', '$location', 'myProvider', '$localStorage', function ($scope, $http, $location, myProvider, $localStorage) {
    ...
    var controlAsignacionCamion;
    var controlAsignacionChofer;
    var msm = '';
...
//funcion que llama a un web services cuando inicializa la pagina
    ...
    $scope.controlAsignacionCamiones = function () {
        ...
        controlAsignacionCamion = false;

        $scope.lista = [];
        $http({
            method: 'POST',
            url: myProvider.getDelivery() + '/getAllDeliveryByFechaVehiculo',
            headers: {
                'Content-Type': 'application/json'
            },
            data: {
            ...
            if (n == 0) {
                ...
                for (var i = 0; i < n; i++) {
                    ...
                    if (($scope.obj.hora_inicio >= auxObj.hora_retorno)) {
                        if(($scope.obj.minutos_inicio >= auxObj.minutos_retorno)){
                            controlAsignacionCamion = true;
                        }else{
                            controlAsignacionCamion = false;
                        }
                    } else {
                ...
                if(controlAsignacionCamion==false){
                    msm = msm + "VEHICULO NO DISPONIBLE. ";
                }

                console.log("SALIMOS DEL POST asignacion camion: " + controlAsignacionCamion);
            }
            $scope.controlAsignacionChofer();
        }, function errorCallback(response) {
            console.log('NO ENTRA CONTROL VEHICULO');
            $scope.mesaje = response.mensaje;
        });
    ...
    $scope.controlAsignacionChofer = function () {
        ...
        controlAsignacionChofer = true;

        $scope.lista = [];
        $http({
            method: 'POST',
            url: myProvider.getDelivery() + '/getAllDeliveryByFechaChofer',
            headers: {
                'Content-Type': 'application/json'
            },
            data: {
            ...
            if (n == 0) {
                ...
                for (var i = 0; i < n; i++) {
                    ...
                    if (($scope.obj.hora_inicio >= auxObj.hora_retorno)) {
                        ...
                        controlAsignacionChofer = false;

                    }

                }

                if(controlAsignacionChofer==false){
                    msm = msm + "CHOFER NO DISPONIBLE. ";
                }
                console.log("SALIMOS DEL POST asignacion chofer: " + controlAsignacionChofer);
            ...
            if(msm==''){
                ...
                alert('ADVERTENCIA: ' + msm);
            }
        }, function errorCallback(response) {
            console.log('NO ENTRA CONTROL CHOFER');
            $scope.mesaje = response.mensaje;
        });
